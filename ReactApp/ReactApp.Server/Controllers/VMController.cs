    ...
    public class VMController : ControllerBase {
        ...
        public async Task<ActionResult<IEnumerable<string>>> Get() {
            try {
                ...
                string query = "SELECT size FROM Instance";
                SqlParameter[] parameters = new SqlParameter[] { };

                // Execute the query asynchronously and retrieve the result
                var vmSizes = await DatabaseAPI.ExtractDataDB(query, parameters, reader => {
        ...
        public async Task<ActionResult<VMData>> GetSizeData(string size) {
            try {
                ...
                var vmData = await DatabaseAPI.ExtractDataDB(query, parameters, reader => {
                    if (reader.Read()) {
                        return new VMData {
                            CPU_Num = reader.GetInt32(reader.GetOrdinal("cpu_num")),
                            GPU_Num = reader.IsDBNull(reader.GetOrdinal("gpu_num")) ? 0 : (decimal?)reader.GetDecimal(reader.GetOrdinal("gpu_num")),
                            ...
                            GPU_TDP = reader.IsDBNull(reader.GetOrdinal("gpu_tdp")) ? 0 : (int?)reader.GetInt32(reader.GetOrdinal("gpu_tdp"))
                        };
                    }
                ...
                return StatusCode(500, $"An error occurred while fetching VM data for size {size}. Please try again later.");
            }
        }
